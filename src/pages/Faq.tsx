...
export default function Faq() {
  ...
  const filtered = faqs.filter(
    (f) =>
      pt(lang, f.qKey).toLowerCase().includes(search.toLowerCase()) ||
      pt(lang, f.aKey).toLowerCase().includes(search.toLowerCase()),
  );

  return (
    <main className="bg-slate-50">
      <Helmet>
        <title>{pt(lang, "faq.title")}</title>
        <meta name="description" content={pt(lang, "faq.metaDesc")} />
        <meta property="og:title" content={pt(lang, "faq.title")} />
        <meta property="og:description" content={pt(lang, "faq.ogDesc")} />
        ...
        <div className="mx-auto w-full max-w-6xl px-5 sm:px-8">
          ...
          <div className="relative mt-8 max-w-lg">
            ...
            <input
              ...
              placeholder={pt(lang, "faq.searchPlaceholder")}
        ...
        <div className="mx-auto w-full max-w-3xl">
          {filtered.length === 0 && (
            <p className="rounded-2xl border border-slate-200 bg-white p-8 text-center text-sm text-slate-500">
              {fmt(pt(lang, "faq.noResults"), search)}
            </p>
          )}
            ...
            {filtered.map((f) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <span className="font-bold text-nova-950">{pt(lang, f.qKey)}</span>
                    <ChevronDown
                  ...
                  >
                    <div className="overflow-hidden">
                      <p className="px-6 pb-5 text-sm leading-relaxed text-slate-600">{pt(lang, f.aKey)}</p>
                    </div>
                  </div>
                </div>
              );
          ...
          <div className="mt-12 overflow-hidden rounded-3xl bg-white shadow-sm">
            <div className="grid md:grid-cols-2">
              <div className="p-8">
                <h2 className="text-xl font-extrabold text-nova-950">{pt(lang, "faq.contactTitle")}</h2>
                ...
                >
                  {pt(lang, "faq.contactButton")} <ArrowRight className="h-4 w-4" />
                </a>
              </div>
              <img
